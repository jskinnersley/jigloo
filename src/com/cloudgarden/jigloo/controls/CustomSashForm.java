...
public class CustomSashForm extends SashForm {
	 ...
	 */
	public static final int NO_MAX_LEFT = 0x1,
		// Custom style bit for not allow max left
	...
	private static class SashInfo {
		...
		// Whether this sashinfo is enabled (i.e. if there is more than one, this will be disabled).
		public int weight = NO_WEIGHT;
		// If slammed to an edge this is the restore weight. -1 means not slammed. This is the weight in the next form (i.e. sash[0] == weight[1].
		...
		// Draw sash border right/bottom		 
		public int[][] sashLocs;
		// There is one entry for each arrow, It is arrowType/arrowDrawn/x/y/height/width of the arrow area.
	...
	// When the sash goes away, its entry is made null.
	protected boolean inMouseClick = false;
	// Because we can't stop drag even when we are in the arrow area, we need
	...
	// Whether cooresponding control needs a sash border

	protected boolean noMaxUp, noMaxDown;

	protected static final int UP_ARROW = 0, UP_MAX_ARROW = 1, DOWN_ARROW = 2, DOWN_MAX_ARROW = 3, MAX_ARROWS = 4;
	...
	public CustomSashForm(Composite parent, int style, int customStyle) {
		...
		if (noMaxUp & noMaxDown)
			return;
		// If you can't max up or down, there there is no need for arrows.

		SASH_WIDTH = 3 + getOrientation() == SWT.VERTICAL ? ARROW_HEIGHT : ARROW_SIZE;

		arrowColor = ColorManager.getColor(99, 101, 156);
		borderColor = ColorManager.getColor(132, 130, 132);

		//This is needed for Eclipse 3.1M4!!!
		layout(false);
...
//		System.err.println("CustomSashForm: Layout "+getOrientation());
		...
		// We have a maximized control, so we don't need to worry about the sash.
...
//			System.err.println("CustomSashForm: Child "+i+", "+children[i]);
		...
		if (newSash == null) {
//			System.err.println("CustomSashForm: NO NEW SASH! "+getOrientation());
		...
		if (currentSashInfo == null || currentSashInfo.sash == null) {
					...
					for (int i = 0; i < currentSashInfo.sashLocs.length; i++) {
						...
						if (locs[locIndex] <= loc && loc <= locs[locIndex] + locs[sizeIndex]) {
							if (currentSashInfo.cursorOver == NO_ARROW) {
								currentSashInfo.sash.setCursor(CursorManager.getCursor(SWT.CURSOR_ARROW));
							}
			...
			newSash.addMouseListener(new MouseAdapter() {
				...
				public void mouseUp(MouseEvent e) {
					...
					currentSashInfo.sash.redraw();
					// Make sure stipple goes away from the mouse up if not over an arrow button.
				}
	...
	protected void recomputeSashInfo() {
		...
		int[] drawArrows = null;
		int[] weights = getWeights();
		// This should be two entries only. We shouldn't of gotton here if there were more than two.
		if (noMaxUp) {
			addArrows = new int[1];
			...
			if (weights[1] == 0) {
				...
				// Since we are in the middle, there is no weight. We've could of been dragged here.
				currentSashInfo.sashBorderLeft = sashBorders != null ? sashBorders[0] : false;
	...
	protected void upMaxClicked(SashInfo sashinfo) {
		...
		// Not currently maxed, save position.
	...
	protected void downMaxClicked(SashInfo sashinfo) {
		...
		if (currentSashInfo.weight == NO_WEIGHT)
			currentSashInfo.weight = weights[1];
		// Not currently maxed, save current weight.
		weights[0] = 1000;
		weights[1] = 0;
	...
	protected void getNewSashArray(SashInfo sashInfo, int[] addArrowTypes, int[] drawArrowTypes) {
		...
		if (vertical) {
			...
			// Center vertically, no margin required.
			width = tSize;
			height = aSize;
		} else {
			...
			// Center horizontally, no margin required.
			width = aSize;
			height = tSize;
		}
	...
	protected void drawArrow(GC gc, int[] sashLoc, boolean selected) {
		...
		if (selected) {
			if (!inMouseClick) {
				...
				gc.drawLine(
					sashLoc[X_INDEX],
					sashLoc[Y_INDEX] + sashLoc[HEIGHT_INDEX],
					sashLoc[X_INDEX],
					sashLoc[Y_INDEX]);
				gc.drawLine(
					sashLoc[X_INDEX],
					sashLoc[Y_INDEX],
					sashLoc[X_INDEX] + sashLoc[WIDTH_INDEX],
					sashLoc[Y_INDEX]);
	...
	protected static final int ARROW_SIZE = 8, ARROW_HEIGHT = 8, ARROW_MARGIN = 3;
	// Margin on each side of arrow
